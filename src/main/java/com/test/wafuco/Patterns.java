...
public class Patterns {
    ...
    Patterns(int[][] src, boolean allow_permutations) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int j = 0; j < count; j++) {
                ...
                if (pattern.top_match(other))
                    valid_connections_t[i].set(j);
                if (pattern.right_match(other))
                    valid_connections_r[i].set(j);
                if (pattern.bottom_match(other))
                    valid_connections_b[i].set(j);
                if (pattern.left_match(other))
                    valid_connections_l[i].set(j);
            }
        }
    ...
    BitSet valid_adjacent_patterns(IntBuffer options, int direction) { // to_direction
        ...
        switch (direction) {
            case 0:
                valid_connections = valid_connections_t;
                break;
            case 1:
                valid_connections = valid_connections_r;
                break;
            case 2:
                valid_connections = valid_connections_b;
                break;
            case 3:
                valid_connections = valid_connections_l;
                break;
            default:
