package com.test.wafuco;

import java.nio.IntBuffer;

import com.test.wafuco.utility.BitSet;
import com.test.wafuco.utility.HeapSet;
import com.test.wafuco.utility.IntStack;
import com.test.wafuco.utility.PriorityQueue;
...
public class WFC {
    ...
    public boolean generate(int[][] output, int failure_limit, boolean wrap_around) {
         ...
         * to store cell pattern indexes. Their capacity is the total number of
         * patterns.
                ...
                if (remaining == 1) {
                    ...
                    while (!propagation_stack.isEmpty()) {
                        ...
                        for (int to_direction = 0; to_direction < 4; to_direction++) {
                            ...
                            if (adjacent_cell.remaining_options() == 1)
                                continue;

                            // sort adjacent cells by entropy (The one with the least will be propagated
                            // first)
    ...
    public void set_noise_seed(int seed) {
        this.noise_seed = seed;
    }

    public void reset_noise_position() {
        noise_position = 1337;
    }
     ...
     * @param buffer           temp internal buffer, used to collect the options
     *                         from the bitset
     ...
     * @param buffer temp internal buffer, used to collect the options from the
     *               bitset
    ...
    private int positive_integer(int max) {
        return positive_integer() % (max + 1);
    }

    private int positive_integer() {
        return hash(++noise_position, noise_seed) & 0x7FFF_FFFF;
    }

    private float white_noise() {
        return positive_integer() / (float) 0x7FFF_FFFF;
    }
    ...
    private static final class Cell implements Comparable<Cell> {
        ...
        void set_entropy(float entropy) {
            this.entropy = entropy;
        }

        void introduce_noise_to_entropy(float noise) {
            entropy += noise;
        }

        void set_remaining_options(int remaining) {
     ...
     * will be pushed first. (So that the one with the least entropy will be popped
     * first)
     */
